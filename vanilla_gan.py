...
def training_loop(train_dataloader, opts):
    ...
    if opts.load:
        try:
            G_path = os.path.join(opts.checkpoint_dir, 'G.pkl')
            D_path = os.path.join(opts.checkpoint_dir, 'D.pkl')
            last_checkpoint_G = torch.load(G_path)
            last_checkpoint_D = torch.load(D_path)
            G.load_state_dict(last_checkpoint_G)
            D.load_state_dict(last_checkpoint_D)
        except:
            print("No checkpoints files to load found")
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = loss(D(real_images), torch.ones(real_images.shape[0])*0.9)
            ...
            D_fake_loss = loss(D(fake_images.detach()), torch.zeros(batch_size))
            ...
            G_loss = loss(D(fake_images), torch.ones(opts.batch_size))
...
def create_parser():
    ...
    parser.add_argument('--load', action='store_true', default=False, help='Whether to load from previous checkpoint')
