...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    if torch.cuda.is_available():
        X_labels_ones = torch.cuda.FloatTensor(opts.batch_size).fill_(1)
        Y_labels_ones = torch.cuda.FloatTensor(opts.batch_size).fill_(1)
        X_labels_zeros = torch.cuda.FloatTensor(opts.batch_size).fill_(0)
        Y_labels_zeros = torch.cuda.FloatTensor(opts.batch_size).fill_(0)
        print("Moved images and labels to GPU")
    else:
        X_labels_ones = torch.ones(opts.batch_size)
        Y_labels_ones = torch.ones(opts.batch_size)
        X_labels_zeros = torch.zeros(opts.batch_size)
        Y_labels_zeros = torch.zeros(opts.batch_size)
        print("Did not move images and labels to GPU")
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        images_X, labels_X = utils.to_var(images_X), utils.to_var(labels_X).float().squeeze()

        images_Y, labels_Y = iter_Y.next()
        images_Y, labels_Y = utils.to_var(images_Y), utils.to_var(labels_Y).float().squeeze()
        ...
        D_X_loss = lossMSE(D_X(images_X), X_labels_ones)
        D_Y_loss = lossMSE(D_Y(images_Y), Y_labels_ones)
        ...
        D_X_loss = lossMSE(D_X(fake_X.detach()), Y_labels_zeros)

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = lossMSE(D_Y(fake_Y.detach()), X_labels_zeros)
        ...
        g_loss = lossMSE(D_X(fake_X), Y_labels_ones)


        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = lossMSE(reconstructed_Y, images_Y)
            g_loss += cycle_consistency_loss

        if opts.use_identity_loss:
            idt_X = G_YtoX(images_X)
            identity_loss = lossMSE(idt_X, images_X)*opts.lr_identity
        ...
        g_loss = lossMSE(D_Y(fake_Y), X_labels_ones)


        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = lossMSE(reconstructed_X, images_X)
            g_loss += cycle_consistency_loss

        if opts.use_identity_loss:
            # generate the identity images by giving images that are in the correct space
            idt_Y = G_XtoY(images_Y)
            # compute identity loss by comparing the identity images to the original images
            identity_loss = lossMSE(idt_Y, images_Y)*opts.lr_identity
            g_loss += identity_loss
...
def create_parser():
    ...
    parser.add_argument('--use_identity_loss', action='store_true', default=False, help='Choose whether to include the identity term in the loss.')
    ...
    parser.add_argument('--lr_identity', type=float, default=0.5, help='The proportinal learning rate of the identity loss with respect to the consistency loss')
...
if __name__ == '__main__':
    ...
    if opts.use_identity_loss:
        opts.sample_dir = '{}_identity'.format(opts.sample_dir)

    if opts.load:
        if opts.load=='checkpoints_cyclegan':
            opts.sample_dir = '{}_checkpoint'.format(opts.sample_dir)
        else:
